...
BUS_STOP_GEOJSON_PATH = Path("LTABusStop.geojson")
OUTPUT_PATH = Path("hdb_bus_stop_features.csv")
        ...
        records.append(
            {
                ...
                "bus_roof_num": properties.get("BUS_ROOF_NUM"),
                "bus_stop_latitude": latitude,
                "bus_stop_longitude": longitude,
            }
        )
...
def attach_nearest_bus_stop_features(hdb_df, bus_stops_df):
    bus_stop_rows = bus_stops_df.to_dict("records")
    enriched_rows = []
    ...
    for hdb_row in hdb_df.to_dict("records"):
        ...
        for bus_stop in bus_stop_rows:
            distance_km = euclidean_distance_km(
                hdb_row["latitude"],
                hdb_row["longitude"],
                bus_stop["bus_stop_latitude"],
                bus_stop["bus_stop_longitude"],
            )
        ...
        enriched_rows.append(
            {
                **hdb_row,
                **nearest_bus_stop,
                "euclidean_distance_to_nearest_bus_stop_km": round(
                    nearest_distance_km, 4
                ),
                "nearest_bus_stop_estimated_walking_km": round(
                    estimated_walking_distance_km, 4
                ),
                "walking_time_to_nearest_bus_stop_mins": round(
...
def main():
    ...
    bus_stops_df = load_bus_stops(BUS_STOP_GEOJSON_PATH)
    feature_df = attach_nearest_bus_stop_features(hdb_df, bus_stops_df)
    feature_df.to_csv(OUTPUT_PATH, index=False)

    print(
        f"Saved {len(feature_df)} HDB rows with bus stop walking-time features to "
        f"{OUTPUT_PATH}"
    )
