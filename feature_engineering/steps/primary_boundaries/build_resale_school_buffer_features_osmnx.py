...
class GoogleDistanceMatrixClient:
    """Distance Matrix walking-distance client without disk cache."""
    ...
    def _fetch_distances_chunk(
        ...
        o_lat, o_lon = origin
        params = {
            "origins": f"{o_lat:.6f},{o_lon:.6f}",
            "destinations": "|".join(f"{d_lat:.6f},{d_lon:.6f}" for d_lat, d_lon in destinations),
            "mode": "walking",
            "units": "metric",
            "key": self.api_key,
        }
        ...
        if status != "OK":
            raise RuntimeError(f"Google Distance Matrix API error: {status}")

        elements = payload.get("rows", [{}])[0].get("elements", [])
        out: list[float | None] = []
        for e in elements:
            if e.get("status") == "OK":
                out.append(float(e["distance"]["value"]))
            else:
                out.append(None)
        return out
    ...
    def get_walking_distances(
        ...
        for start in range(0, len(destinations), max_destinations_per_request):
            chunk_destinations = destinations[start : start + max_destinations_per_request]
            dists = self._fetch_distances_chunk((o_lat, o_lon), chunk_destinations)
            for idx_local, dist in enumerate(dists):
                out[start + idx_local] = dist
...
def add_mall_access_features(
    ...
    distance_provider: str = "google",
    google_client: GoogleDistanceMatrixClient | None = None,
    google_nearest_candidate_k: int = 25,
    ...
    if provider == "google":
        if google_client is None:
            raise ValueError("google_client is required when distance_provider='google'")
        ...
        for i, (origin_xy, origin_ll) in enumerate(zip(point_xy, point_latlon)):
            ...
            count_distances = google_client.get_walking_distances(tuple(origin_ll), count_destinations)
            within_count = sum(
                1 for d in count_distances if d is not None and d <= float(walk_distance_10min_m)
            )
            within_counts.append(int(within_count))

            # Candidate set for nearest mall: top-k by euclidean as practical Google call budget control.
            euclid_d = np.sqrt(((mall_xy - origin_xy) ** 2).sum(axis=1))
            sort_idx = np.argsort(euclid_d)
            if google_nearest_candidate_k and google_nearest_candidate_k > 0:
                sort_idx = sort_idx[: int(google_nearest_candidate_k)]

            near_destinations = [tuple(mall_latlon[idx]) for idx in sort_idx]
            near_distances = google_client.get_walking_distances(tuple(origin_ll), near_destinations)
            ...
            if best_idx is None:
                # Fallback if no Google route returned.
...
def add_mrt_access_features(
    ...
    distance_provider: str = "google",
    google_client: GoogleDistanceMatrixClient | None = None,
    google_nearest_candidate_k: int = 40,
    osmnx_graph: nx.MultiDiGraph | None = None,
) -> gpd.GeoDataFrame:
    out = matched_points.copy()
    ...
    if provider == "google":
        ...
        for i, (origin_xy, origin_ll) in enumerate(zip(point_xy, point_latlon)):
            ...
            # first candidate exits by euclidean lower bound, then true walking filter via Google.
            count_candidates = tree.query_ball_point(origin_xy, r=radius_for_count)
...
def main() -> None:
    ...
    parser.add_argument(
        "--distance-provider",
        choices=["google", "osmnx", "euclidean"],
        default="osmnx",
    ...
    parser.add_argument(
        "--google-request-sleep-seconds",
        type=float,
        default=0.0,
        help="Optional delay between Google API requests",
    )
    parser.add_argument(
        "--google-timeout-seconds",
        type=int,
        default=30,
        help="HTTP timeout for Google API requests",
    )
    parser.add_argument(
        "--google-nearest-candidate-k",
        type=int,
        default=25,
        help="Top-K nearest euclidean candidates to evaluate via Google for nearest-distance fields",
    )
    parser.add_argument(
        "--osmnx-network-type",
        default="walk",
    ...
    if args.distance_provider == "google" and not args.google_maps_api_key:
        raise ValueError(
            "Google distance provider selected but no API key was provided. "
            "Set GOOGLE_MAPS_API_KEY or pass --google-maps-api-key."
        )

    google_client = None
    osmnx_graph = None
    if args.distance_provider == "google":
        google_client = GoogleDistanceMatrixClient(
            api_key=str(args.google_maps_api_key),
            request_sleep_seconds=float(args.google_request_sleep_seconds),
            timeout_seconds=int(args.google_timeout_seconds),
        )
    ...
    matched_points = add_mall_access_features(
        ...
        google_nearest_candidate_k=args.google_nearest_candidate_k,
    ...
    matched_points = add_mrt_access_features(
        ...
        distance_provider=args.distance_provider,
        google_client=google_client,
        google_nearest_candidate_k=max(int(args.google_nearest_candidate_k), 40),
        osmnx_graph=osmnx_graph,
    )
