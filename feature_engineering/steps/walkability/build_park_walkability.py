...
PARK_GEOJSON_PATH = Path("Parks.geojson")
OUTPUT_PATH = Path("hdb_park_features.csv")
        ...
        records.append(
            {
                ...
                "park_object_id": properties.get("OBJECTID"),
...
def attach_nearest_park_features(hdb_df, parks_df):
    park_rows = parks_df.to_dict("records")
    enriched_rows = []
        ...
        enriched_rows.append(
            {
                **hdb_row,
                **nearest_park,
                "euclidean_distance_to_nearest_park_km": round(
                    nearest_distance_km, 4
                ),
                "nearest_park_estimated_walking_km": round(
                    estimated_walking_distance_km, 4
                ),
                "walking_time_to_nearest_park_mins": round(walking_time_minutes, 2),
            }
        )
...
def main():
    ...
    parks_df = load_parks(PARK_GEOJSON_PATH)
    feature_df = attach_nearest_park_features(hdb_df, parks_df)
    feature_df.to_csv(OUTPUT_PATH, index=False)

    print(
        f"Saved {len(feature_df)} HDB rows with park walking-time features to "
        f"{OUTPUT_PATH}"
    )
